...
export default function AdminPage() {
  ...
  const handleLogin = (e) => {
    ...
    // Accepts either 8899 or Gomatha@2026
    const cleanPass = password.trim();
    if (cleanPass === '8899' || cleanPass === 'Gomatha@2026') {
      setIsAuthenticated(true);
      sessionStorage.setItem('gomatha_admin_auth', 'true');
      sessionStorage.setItem('gomatha_admin_pass', cleanPass);
      fetchProducts();
    } else {
      setLoginError('Invalid password! Enter 8899 or Gomatha@2026');
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingId) {
        ...
        setStatusMessage('Saving product to catalog...');
    ...
    return (
      <div className="min-h-[80vh] flex items-center justify-center p-4">
        ...
        >
          ...
          <div>
            <h2 className="text-xl font-black tracking-tight text-stone-900 font-sans">
              gomatha <span className="text-[#9f2089]">admin</span>
            </h2>
            <p className="text-xs text-stone-500 mt-1">
              Enter admin password (<strong>8899</strong> or <strong>Gomatha@2026</strong>) to upload and manage inventory.
            </p>
          </div>
          ...
          >
            {isVerifying ? 'Checking...' : 'Unlock Admin Portal'}
          </button>
    ...
    <div className="max-w-xl mx-auto p-4 sm:p-6 pb-24 space-y-8">
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className="inline-flex items-center gap-1.5 text-xs font-semibold text-rose-600 hover:text-rose-800 bg-rose-50 px-3 py-1.5 rounded-lg transition"
        >
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-stone-200 p-5 sm:p-6">
        ...
        <p className="text-xs text-stone-500 mb-4">
          {editingId ? 'Update price, stock, or description.' : 'Snap photos, set stock, and publish.'}
        </p>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          {!editingId && (
            <div>
              ...
              <div className="relative border-2 border-dashed border-stone-300 hover:border-[#9f2089] rounded-xl p-4 text-center cursor-pointer transition">
                <div className="py-4 flex flex-col items-center space-y-1">
                  <Camera className="w-7 h-7 text-[#9f2089]" />
                  <span className="text-xs font-medium text-stone-700">Tap to snap or select photos</span>
                  <span className="text-[10px] text-stone-400">Multiple images allowed</span>
                </div>
                <input
                  type="file"
                  accept="image/*"
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <select
                ...
                className="w-full p-2.5 text-xs rounded-lg border border-stone-300 bg-white focus:outline-none focus:border-[#9f2089]"
              >
            ...
            <div>
              ...
              <input
                ...
                className="w-full p-2.5 text-xs rounded-lg border border-stone-300 focus:outline-none focus:border-[#9f2089]"
                required
              />
            </div>
          </div>
          ...
          <div>
            ...
            <textarea
              rows={2}
              placeholder="Material details, size specifications, fabric type..."
              value={description}
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full bg-[#9f2089] hover:bg-[#851871] text-white font-bold py-3 rounded-xl text-xs sm:text-sm transition flex items-center justify-center gap-2"
          >
      ...
      {/* Catalog Manager */}
      <div className="bg-white rounded-2xl shadow-sm border border-stone-200 p-5 sm:p-6">
        <h2 className="text-base font-bold text-stone-900 mb-3">Manage Catalog ({products.length})</h2>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2 text-[11px] mt-0.5">
                    <span className="text-[#9f2089] font-bold">₹{p.price}</span>
